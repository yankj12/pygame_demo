...
class AiEntity(object):
    ...
    def __init__(self, k_max_rows, k_max_cols):
        ...
        self.pattern = [[0] * self.k_max_cols] * self.k_max_rows
    ...
    def build_path_segment(self, start_point, end_point):
        ...
        delta_row = current_point.row - end_point.row
        delta_col = current_point.col - end_point.col

        while delta_row != 0 and delta_col != 0:
...
while True:
    ...
    if current_second - last_second >= 0.5:
        ...
        next_point = move_pattern.follow_pattern(record_path)
        ball.row = next_point.row
        ball.col = next_point.col

        record_path.append(PathPoint(next_point.row, next_point.col))
